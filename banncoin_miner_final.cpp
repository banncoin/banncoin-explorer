...
int detect_start_height() {
    int highest = -1;
    for (const auto& entry : directory_iterator(".")) {
        string name = entry.path().filename().string();
        if (name.rfind("block", 0) == 0 && name.find(".json") != string::npos) {
            try {
                int height = stoi(name.substr(5, name.find(".json") - 5));
...
int main(int argc, char* argv[]) {
    ...
    int block_height = detect_start_height();
